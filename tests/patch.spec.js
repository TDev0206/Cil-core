...
describe('PatchDB', () => {
    ...
    describe('_mergeMapsTxBlocks', async () => {
        it('should merge only first map', async () => {
            const strTxHash = pseudoRandomBuffer().toString('hex');
            const strBlockHash = pseudoRandomBuffer().toString('hex');

            const patch = new factory.PatchDB(0);
            patch.bindTxToBlock(strTxHash, strBlockHash);
            const patch2 = new factory.PatchDB(1);
            const patchResult = new factory.PatchDB(1);

            patch._mergeMapsTxBlocks(patch2, patchResult);

            assert.deepEqual(Array.from(patchResult._mapTxBlock), [[strTxHash, strBlockHash]]);
        });

        it('should merge only second map', async () => {
            const strTxHash = pseudoRandomBuffer().toString('hex');
            const strBlockHash = pseudoRandomBuffer().toString('hex');

            const patch = new factory.PatchDB(0);
            const patch2 = new factory.PatchDB(1);
            patch2.bindTxToBlock(strTxHash, strBlockHash);
            const patchResult = new factory.PatchDB(1);
        ...
        it('should merge both', async () => {
            const strTxHash = pseudoRandomBuffer().toString('hex');
            const strBlockHash = pseudoRandomBuffer().toString('hex');
            const strTxHash2 = pseudoRandomBuffer().toString('hex');
            const strBlockHash2 = pseudoRandomBuffer().toString('hex');

            const patch = new factory.PatchDB(0);
            patch.bindTxToBlock(strTxHash, strBlockHash);
            const patch2 = new factory.PatchDB(1);
            patch2.bindTxToBlock(strTxHash2, strBlockHash2);
            ...
            assert.deepEqual(
                Array.from(patchResult._mapTxBlock),
                [
                    [strTxHash, strBlockHash],
                    [strTxHash2, strBlockHash2]
                ]
            );
        });
    });

    describe('fixTxToBlockMap', async () => {
        it('should set fix all delayed txns', async () => {
            const strTxHash = pseudoRandomBuffer().toString('hex');
            const strTxHash2 = pseudoRandomBuffer().toString('hex');
            const strBlockHash = pseudoRandomBuffer().toString('hex');

            const patch = new factory.PatchDB(0);
            patch.bindTxToBlock(strTxHash, null);
            patch.bindTxToBlock(strTxHash2, null);
            assert.deepEqual(
                Array.from(patch._mapTxBlock),
                [
                    [strTxHash, null],
                    [strTxHash2, null]
                ]
            ...
            patch.fixTxToBlockMap(strBlockHash);

            assert.deepEqual(
                Array.from(patch._mapTxBlock),
                [
                    [strTxHash, strBlockHash],
                    [strTxHash2, strBlockHash]
                ]
            );
        });
    });
    ...
    describe('Merge', async () => {
        ...
        it('should FAIL to merge patches (same outputs same spending TX different blocks)', async () => {
            const spendingTx = pseudoRandomBuffer();

            const patch = new factory.PatchDB(12);
            patch.bindTxToBlock(spendingTx.toString('hex'), pseudoRandomBuffer().toString('hex'));

            const patch2 = new factory.PatchDB(0);
            patch2.bindTxToBlock(spendingTx.toString('hex'), pseudoRandomBuffer().toString('hex'));

            const utxo = createUtxo([12, 0, 431]);
            patch.spendCoins(utxo.clone(), 12, spendingTx);
            patch2.spendCoins(utxo.clone(), 12, spendingTx);

            const strErr = `Tx "${spendingTx.toString('hex')}" exists in`;
            assert.throws(() => patch.merge(patch2), new RegExp(strErr));
        });
        ...
        it('should MERGE patches and maintain _mapSpentUtxos (2 patches, same utxo)', async () => {
            ...
            patch.bindTxToBlock(spendingTx2.toString('hex'), pseudoRandomBuffer().toString('hex'));
            ...
            function test(patchMerged) {
                ...
                assert.equal(patchMerged._mapTxBlock.size, 2);
            }
        });
        ...
        it('should MERGE patches and maintain _mapSpentUtxos (empty patch)', async () => {
            const patch = new factory.PatchDB(12);
